...
class Sphere:
    ...
    def update(self,t):
        ...
        for i in range(0, self.detalization):
            phi = graduceToRad((i / self.detalization) * 360)
            for j in range(0, self.detalization):
                tetha = graduceToRad((j / (self.detalization-1)) * 180)
    ...
    def draw(self, t, camera_pos, focal_length, hweight, hheight):
        ...
        for c in self.faces_to_draw:
            elem = self.face_vertexesSCR[c[3]]
            self.drawscreen.create_polygon(elem[0], elem[1], elem[2], elem[3], fill = rgb_to_hex(rgb_unit0_1(10/c[4])))