...
class Cube:
    ...
    def draw(self, t, camera_pos, focal_length, hweight, hheight, mode, zblackout, lightpos, normalized_light_dir):
        ...
        for i in self.cube_facesC:
            ...
            litcoeff = max(dotproduct(normal, normalized_light_dir), 0)
...
class Sphere:
    ...
    def draw(self, t, camera_pos, focal_length, hweight, hheight, mode, zblackout, lightpos, normalized_light_dir):
        ...
        for i in self.sphere_facesC:
            ...
            litcoeff = max(dotproduct(normal, normalized_light_dir)*-1, 0)
        ...
        for c in self.faces_to_draw:
            if True:
                elem = self.face_vertexesSCR[c[3]]
                self.drawscreen.create_polygon(elem, fill = (rgb_to_hex((round(abs(sin(c[3]))*255), round(abs(cos(c[3])*255)), round(abs(sin(c[3]+4)*255))), 1) if mode["Index of faces"] else rgb_to_hex(rgb_unit0_1(c[5]))))
